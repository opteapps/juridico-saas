...
export function JurimetriaPage() {
  ...
  const porAreaData = dashboard?.porArea?.map((a: any) => ({
    name: a.area || 'Sem área',
    value: a._count,
  })) || []

  const porStatusData = dashboard?.porStatus?.map((s: any) => ({
    name: s.status,
    value: s._count,
  })) || []
              ...
              <ResponsiveContainer width="100%" height={280}>
                <PieChart>
                  <Pie data={porAreaData} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={100} label={({ name, percent }) => `${name} (${(percent * 100).toFixed(0)}%)`}>
                    {porAreaData.map((_: any, i: number) => <Cell key={i} fill={CORES[i % CORES.length]} />)}
                  </Pie>
                  <Tooltip />
                </PieChart>
              </ResponsiveContainer>
