...
export function SAEscritoriosPage() {
  ...
  const suspender = useMutation({
    mutationFn: ({ id, ativo }: { id: string; ativo: boolean }) => api.patch(`/super-admin/tenants/${id}/status`, { ativo }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['sa-tenants'] }),
  })
            ...
            {isLoading ? (
              ...
              <tr key={tenant.id} className="hover:bg-gray-50">
                <td className="px-6 py-4">
                  ...
                  {tenant.cnpj && <p className="text-gray-400 text-xs">{tenant.cnpj}</p>}
                </td>
                <td className="px-6 py-4">
                  <span className="px-2 py-1 bg-blue-50 text-blue-700 text-xs rounded-full font-medium">{tenant.plano?.nome}</span>
                  <p className="text-gray-400 text-xs mt-1">R$ {tenant.plano?.preco}/mês</p>
                </td>
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3 text-gray-600">
                    <span className="flex items-center gap-1"><Users className="h-3 w-3" />{tenant._count?.usuarios}</span>
                    <span className="flex items-center gap-1"><FileText className="h-3 w-3" />{tenant._count?.processos}</span>
                  </div>
                </td>
                <td className="px-6 py-4 text-gray-500">{formatarData(tenant.criadoEm)}</td>
                <td className="px-6 py-4">
                  <span className={`px-2 py-1 text-xs rounded-full font-medium ${tenant.ativo ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
                    {tenant.ativo ? 'Ativo' : 'Suspenso'}
                  </span>
                </td>
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    <Link to={`/super-admin/escritorios/${tenant.id}`} className="p-1.5 hover:bg-gray-100 rounded" title="Ver detalhes">
                      <Eye className="h-4 w-4 text-gray-500" />
                    </Link>
                    <button
                      onClick={() => suspender.mutate({ id: tenant.id, ativo: !tenant.ativo })}
                      className={`p-1.5 hover:bg-gray-100 rounded ${tenant.ativo ? 'text-red-500' : 'text-green-500'}`}
                      title={tenant.ativo ? 'Suspender' : 'Reativar'}
                    >
