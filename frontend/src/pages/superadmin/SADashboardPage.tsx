  ...
  return (
    <div className="p-8 space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((s) => (
          <div key={s.label} className="bg-white rounded-xl border p-6 shadow-sm">
            <div className={`inline-flex p-3 rounded-lg ${s.bg} mb-4`}>
              <s.icon className={`h-6 w-6 ${s.color}`} />
            </div>
            <p className="text-2xl font-bold text-gray-900">{s.value}</p>
            {s.total && <p className="text-xs text-gray-400">{s.total} total cadastrados</p>}
        ...
        {data?.distribuicaoPlanos?.length > 0 && (
          <div className="bg-white rounded-xl border p-6 shadow-sm">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <PieChart>
                <Pie
                  data={data.distribuicaoPlanos}
                  dataKey="total"
                  nameKey="plano"
                  cx="50%"
                  cy="50%"
                  outerRadius={80}
                  label={({ plano, total }: { plano: string; total: number }) => `${plano}: ${total}`}
                >
                  {data.distribuicaoPlanos.map((_: unknown, i: number) => (
